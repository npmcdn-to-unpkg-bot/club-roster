	...
	var table = $( '#' + options.id ).on( 'init.dt', function( e, settings, json ){
		if ( options.individualSearching == true ) {
			...
			for ( var c = 0; c < options.columns.length; c++ ) {
				...
				if( table.DataTable().column( c ).visible() !== true ){
					$( td ).css( 'display', 'none' );
				}
				
				if ( options.columns[ c ].searchable ) {
					
					// optionally override the search input box 
					// id and html both required
					if( options.columns[ c ].searchInput ){
						if( options.columns[ c ].searchInput.id == undefined ){
							$.error( 'id not defined for search input for column ' + JSON.stringify( options.columns[ c ] ) );
						} else if( options.columns[ c ].searchInput.html == undefined ){
							$.error( 'html not defined for search input for column ' + JSON.stringify( options.columns[ c ] ) );
						} else {
							$( td ).append( options.columns[ c ].searchInput.html );
							$( '#' + options.columns[ c ].searchInput.id, '#' + options.id ).keyup( function(){
						...
						$( div ).append( label );
						$( label ).attr( 'for', options.id + '-dtSearch-' + c );
						$( label ).addClass( 'sr-only' );
						$( label ).html( options.columns[ c ].placeholder || options.columns[ c ].title || options.columns[ c ].name );
						
						var input = document.createElement( 'input' );
						input.type = 'text';
						$( div ).append( input );
						
						$( input ).attr( 'id', options.id + '-dtSearch-' + c );
						$( input ).attr( 'autocomplete', 'off' );
						$( input ).addClass( 'form-control' );
						$( input ).attr( 'placeholder', options.columns[ c ].placeholder || options.columns[ c ].title || options.columns[ c ].name );
						
						$( input ).keyup( function( e ){
							// TODO maybe check event key code for shift, enter, ctrl... 
							table.DataTable().draw();
						});
					}
				}
		...
		dom 			: options.colVis == true ? "<'row'<'col-xs-6'l><'col-xs-6'fC>>" + "rt" + "<'row'<'col-xs-6'i><'col-xs-6'p>>" : options.dom,
		...
		ajax : {
			...
			data : function( object ){
				...
				for ( var c = 0; c < options.columns.length; c++ ) {
					if ( options.individualSearching == true && options.columns[ c ].searchable && options.columns[ c ].searchInput != undefined && ( "#" + options.columns[ c ].searchInput.id, '#' + options.id ).length > 0 ) {
						if( $( "#" + options.columns[ c ].searchInput.id, '#' + options.id ).val() && $( "#" + options.columns[ c ].searchInput.id, '#' + options.id ).val().trim().length > 0 ){
							object.columns[ c ].search = { 'regex' : false, 'value' : $( "#" + options.columns[ c ].searchInput.id, '#' + options.id ).val() };
						} else {
							object.columns[ c ].search = { 'regex' : false, 'value' : '' };
						}
					} else if ( options.individualSearching == true && options.columns[ c ].searchable && ( "#" + options.id + "-dtSearch-" + c, '#' + options.id ).length > 0 ) {
						if( $( "#" + options.id + "-dtSearch-" + c, '#' + options.id ).val() && $( "#" + options.id + "-dtSearch-" + c, '#' + options.id ).val().trim().length > 0 ){
							object.columns[ c ].search = { 'regex' : false, 'value' : $( "#" + options.id + "-dtSearch-" + c, '#' + options.id ).val() };
						} else {
							object.columns[ c ].search = { 'regex' : false, 'value' : '' };
						}
					} else {
