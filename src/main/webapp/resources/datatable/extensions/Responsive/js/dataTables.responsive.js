...
( function( window, document, undefined ){
	...
	var factory = function( $, DataTable ){
		...
		Responsive.prototype = {
			...
			_columnsVisiblity : function( breakpoint ){
				...
				var display = $.map( columns, function( col ){
					return col.auto && col.minWidth === null ? false : col.auto === true ? '-' : col.includeIn.indexOf( breakpoint ) !== -1;
				} );
			...
			_resizeAuto : function(){
				...
				dt.rows( {
					...
					if( row != null ){
						var node = row.node();
						// node is null when server side datatable is empty
						if( node != null ){
			...
			details : {
				renderer : function( api, rowIdx ){
					var data = api.cells( rowIdx, ':hidden' ).eq( 0 ).map( function( cell ){
						...
						return '<li>' + '<span class="dtr-title">' + header.text() + ':' + '</span> ' + '<span class="dtr-data">' + api.cell( cell ).data() + '</span>' + '</li>';
					} ).toArray().join( '' );
		...
		$( document ).on( 'init.dt.dtr', function( e, settings, json ){
			if ( $( settings.nTable ).hasClass( 'responsive' ) || $( settings.nTable ).hasClass( 'dt-responsive' ) || settings.oInit.responsive ) {
